...
impl KVClient {
    ...
    pub async fn new(db_id: Option<usize>, caller_mem: Option<i32>) -> Self {
        ...
        let caller_mem = caller_mem.or(Some(512));
        let fc =
            FunctionalClient::new("csqlite", "rkvactor", Some(db_id), caller_mem.clone()).await;
        KVClient {
            fc: Arc::new(fc),
            caller_mem: caller_mem.unwrap(),
        }
...
mod tests {
    ...
    use crate::{
        bench::{KEY_SIZE_B, NUM_KEYS, START_KEY, VAL_SIZE_B},
        BenchFn,
    };
    ...
    async fn run_simple_scaling_test() {
        ...
        let cl = KVClient::new(Some(0), Some(20000)).await;
        let resp = cl
            .txn(
                vec![],
                vec![("Key0".into(), Some("Val0".into()))],
                vec![],
                false,
            )
            .await;
        println!("Init Resp: {resp:?}");
        for i in 0..10000 {
            let k = i % 10;
            ...
            let resp = cl
                .txn(vec![], vec![(key.clone(), Some(val))], vec![], false)
                .await;
            println!("Put Resp: {resp:?}. Duration: {:?}.", start_time.elapsed());

            let start_time = std::time::Instant::now();
            let resp = cl.txn(vec![], vec![], vec![key], false).await;
            println!("Get Resp: {resp:?}. Duration: {:?}", start_time.elapsed());
        }
    }
    ...
    async fn run_kv_datagen_test() {
        ...
        if reset {
            ...
            while i < end_key {
                ...
                let updates = keyvals
                    ...
                    .collect::<Vec<_>>();
                loop {
                    let r = kvcl.txn(vec![], updates.clone(), vec![], true).await;
                    if r.is_ok() {
                        println!("Ok.");
                        break;
                    } else {
        ...
        let resp = kvcl.txn(vec![], vec![], vec![key], false).await;
        println!("Resp: {resp:?}");
    }
}
